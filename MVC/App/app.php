...
try {
    ...
    valid()->addMessage('CREATETABLE','Unable to create table book '.$database->lastError(),'info');
    // we try to create the schema. The user must have permission to create schema.
}
...
function router() {
    ...
    if ($router===null) {
        $router=new RouteOne();
        $router->setDefaultValues('Book','Home'); // the default controller is Ticket and the default action is Index.
